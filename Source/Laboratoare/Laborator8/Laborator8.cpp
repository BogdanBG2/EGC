...
void Laborator8::Update(float deltaTimeSeconds) {
	{
		...
		RenderSimpleMesh(meshes["sphere"], shaders["ShaderLab8"], modelMatrix, glm::vec3(0, 0.7f, 1));
	}

	{
		...
		RenderSimpleMesh(meshes["box"], shaders["ShaderLab8"], modelMatrix, glm::vec3(1, 0, 0.15f));
	}
...
void Laborator8::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color) {
	...
	// Bind cut-off angle
	float cut_off_angle = glGetUniformLocation(shader->program, "cut_off_angle");
	glUniform1f(cut_off_angle, cutOff);
...
void Laborator8::OnInputUpdate(float deltaTime, int mods) {
	...
	if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
	{
		...
		if (window->KeyHold(GLFW_KEY_UP))
			xAngle += deltaTime * speed;
		if (window->KeyHold(GLFW_KEY_DOWN))
			xAngle -= deltaTime * speed;
		if (window->KeyHold(GLFW_KEY_LEFT))
			yAngle += deltaTime * speed;
		if (window->KeyHold(GLFW_KEY_RIGHT))
			yAngle -= deltaTime * speed;

		// Change the cut-off angle
		if (window->KeyHold(GLFW_KEY_I))
			cutOff = min(180.0f, cutOff + speed * 50 * deltaTime);
		if (window->KeyHold(GLFW_KEY_K))
			cutOff = max(0.0f, cutOff - speed * 50 * deltaTime);

		// Change the light direction
		glm::mat4 turnAngle = glm::mat4(1);
		turnAngle = glm::rotate(turnAngle, yAngle, glm::vec3(0, 1, 0));
		turnAngle = glm::rotate(turnAngle, xAngle, glm::vec3(1, 0, 0));

		lightDirection = glm::vec3(turnAngle * glm::vec4(0, -1, 0, 0));
	}
}
