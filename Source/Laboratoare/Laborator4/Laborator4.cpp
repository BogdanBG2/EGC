...
void Laborator4::Update(float deltaTimeSeconds)
{
	...
	{
		if (jump) {
			jumpTime += deltaTimeSeconds;
			jumpZ = crtZ - jumpTime;

			jumpAngle -= M_PI / 2 * deltaTimeSeconds;

			jumpY = 0.5f + 4 * jumpTime - 4 * jumpTime * jumpTime;
			...
			if (jumpTime >= 1) {
				...
				crtZ = jumpZ;
				jumpTime = 0;
				jumpAngle = M_PI / 2 * ++jumps;
			}
		}

		modelMatrix = glm::mat4(1);
		modelMatrix *= Transform3D::Translate(0, jumpY, jumpZ);
		modelMatrix *= Transform3D::RotateOX(jumpAngle);
		RenderMesh(meshes["box"], shaders["VertexNormal"], modelMatrix);
	}
	...
	{
		// Sun
		solarTime += 2 * deltaTimeSeconds;
		solarSystemX = sin(solarTime);
		modelMatrix = glm::mat4(1);
		modelMatrix *= Transform3D::Translate(solarSystemX, 2.5, 0);
		RenderMesh(meshes["sphere"], shaders["VertexNormal"], modelMatrix);

		// Planet
		angularStep += deltaTimeSeconds;
		modelMatrix *= Transform3D::Scale(0.5f, 0.5f, 0.5f);
		modelMatrix *= Transform3D::RotateOY(angularStep * 0.6f);
		modelMatrix *= Transform3D::Translate(2.6f, 0, 2.6f);
		RenderMesh(meshes["box"], shaders["Color"], modelMatrix);

		// Satellite
		modelMatrix *= Transform3D::Scale(0.38f, 0.38f, 0.38f);
		modelMatrix *= Transform3D::RotateOY(angularStep * 3.2f);
		modelMatrix *= Transform3D::Translate(2.6f, 0, 2.6f);
		RenderMesh(meshes["box"], shaders["Simple"], modelMatrix);
	}
}
