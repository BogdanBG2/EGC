...
void Laborator9::Update(float deltaTimeSeconds)
{
...

	{
		...
		glm::vec3 OY = glm::mat3(GetSceneCamera()->GetViewMatrix()) * glm::vec3(0, 0, 1);
		float angle = acos(glm::dot(OY, glm::vec3(0, 0, 1)));
...
void Laborator9::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, Texture2D* texture1, Texture2D* texture2) {
	...
	bool isQuad = (mesh == meshes["square"]);
	int localIsQuad = glGetUniformLocation(shader->program, "isQuad");
	glUniform1i(localIsQuad, isQuad);

	int localTime = glGetUniformLocation(shader->program, "Time");
	if (mesh == meshes["sphere"])
		glUniform1f(localTime, Engine::GetElapsedTime());
	else
