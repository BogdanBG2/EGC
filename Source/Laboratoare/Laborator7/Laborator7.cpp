...
{
	{
		...
		RenderSimpleMesh(meshes["sphere"], shaders["ShaderLab7"], modelMatrix, glm::vec3(0, 0.45f, 1));
	}

	{
		...
		RenderSimpleMesh(meshes["box"], shaders["ShaderLab7"], modelMatrix, glm::vec3(255.f / 256, 187.f / 256, 51.f / 256));
	}

	{
		glm::mat4 modelMatrix = glm::mat4(1);
		modelMatrix = glm::translate(modelMatrix, glm::vec3(-2, 1.2f, 0));
		modelMatrix = glm::rotate(modelMatrix, RADIANS(60.0f), glm::vec3(1, 1, 0));
		RenderSimpleMesh(meshes["box"], shaders["ShaderLab7"], modelMatrix, glm::vec3(0, 1, 0.25f));
	}
...
void Laborator7::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
	...
	int localLightPos = glGetUniformLocation(shader->program, "light_position");
	glUniform3fv(localLightPos, 1, glm::value_ptr(lightPosition));

	// TODO: Set eye position (camera position) uniform
	glm::vec3 eyePosition = GetSceneCamera()->transform->GetWorldPosition();
	int localEyePos = glGetUniformLocation(shader->program, "eye_position");
	glUniform3fv(localEyePos, 1, glm::value_ptr(eyePosition));
	...
	int locMaterialKd = glGetUniformLocation(shader->program, "material_kd");  // the diffuse light constant
	glUniform1f(locMaterialKd, materialKd);

	int locMaterialKs = glGetUniformLocation(shader->program, "material_ks");  // the specular light constant
